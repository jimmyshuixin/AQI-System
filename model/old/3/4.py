...
#    - 训练阶段采用基于IQR（四分位距）的异常值识别与插值平滑技术。
#    - 高级特征工程:
...
#
# 系统设计理念与优化考量:
# - 统一化接口设计: AQISystem 类作为系统所有功能的中心化调用入口，简化用户操作。
# - 模块化与高内聚: ModelTrainer 和 ModelPredictor (其核心逻辑被AQISystem部分集成或直接调用) 各司其职，负责特定的核心任务。
# - 数据处理的鲁棒性: 特别加强了数据预处理流程的稳健性，以应对真实世界数据中常见的噪声和不完整性。
# - 预测精度与系统实用性: 通过集成先进的预测模型与实用的异常检测功能，提升系统的整体价值。
# ==============================================================================
...
# --- 全局核心参数定义 ---
DEFAULT_FILE_PATH = '南京_AQI_Data.xlsx' # 默认训练数据文件的相对或绝对路径
DEFAULT_LOOK_BACK = 720 # 模型回溯历史数据的时间窗口长度（单位：小时）
...
# --- 模型及相关文件保存路径与命名规范 ---
MODEL_ARTIFACTS_DIR = "model_artifacts_advanced" # 存储所有模型相关文件（权重、配置、缩放器等）的目录名
...
class ModelTrainer:
    ...
    def _load_and_preprocess_data_core(self, file_path, fit_scalers=True):
        ...
        5.  （仅训练时）基于IQR的异常值检测与插值修复：针对目标列进行，以平滑极端值。
        6.  特征工程：
        ...
        # 5. 复杂异常值处理 (仅在训练并拟合新缩放器时，对目标列进行IQR异常检测和插值)
        if fit_scalers: 
    ...
    def _objective_optuna(self, trial, X_train_np, y_train_np, X_val_np, y_val_np, num_input_features):
        ...
        scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=scheduler_factor, patience=scheduler_patience, verbose=False) # verbose=False 减少日志输出
...
class AQISystem:
    ...
    def _load_default_config(self):
        ...
        return {
            ...
            'anomaly_threshold_factor': DEFAULT_ANOMALY_THRESHOLD_FACTOR # 新增：异常检测阈值因子
        }
