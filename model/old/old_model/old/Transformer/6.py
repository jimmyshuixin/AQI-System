...
from keras.layers import Dense, Dropout, MultiHeadAttention, LayerNormalization, Flatten, Embedding # 更新导入
...
class AQIPredictor:
    def __init__(self, data_path, model_path="trained_model.h5", sequence_length=24, lag_steps=3, rolling_windows=[3, 6]):
        ...
        self.initial_history_df_for_future_features = None 
    ...
    def _add_time_features(self, df_input):
        ...
        if 'datetime' not in df.columns:
            if self.date_col_name in df.columns and self.hour_col_name in df.columns:
                 ...
                 df['datetime'] = pd.to_datetime(df['datetime_str_temp'], format='%Y%m%d%H')
            else:
                raise ValueError("DataFrame must contain 'datetime' column or 'date' and 'hour' columns to create time features.")
    ...
    def _load_and_prepare_data(self, file_path=None, is_external_test_set=False):
        ...
        if not is_external_test_set and not self.feature_names: 
        ...
        if not is_external_test_set:
            ...
            # Need enough history to calculate all lags and rolling windows for the first *predicted* step's features
            buffer_len = max(self.lag_steps, max(self.rolling_windows) if self.rolling_windows else 0)
            history_needed_for_features = self.sequence_length + buffer_len 
            self.initial_history_df_for_future_features = aqi_data[['datetime', self.hour_col_name] + self.target_names].iloc[-history_needed_for_features:].copy()
    ...
    def predict_future(self, model, city_name, output_dir, num_steps=72):
        ...
        # It needs to be initialized with enough history to calculate lags and rolling stats for the first *predicted* step
        history_df_for_features = self.initial_history_df_for_future_features.copy()
        ...
        last_known_datetime_for_pred_start = self.true_absolute_last_datetime # Use the true last datetime from original data
        if last_known_datetime_for_pred_start is None: 
            last_known_datetime_for_pred_start = pd.Timestamp.now().replace(minute=0,second=0,microsecond=0)
            print(f"警告: 未能确定真实最后时间戳，将使用当前时间 {last_known_datetime_for_pred_start} 作为未来预测的基准。")
        
        future_timestamps = pd.date_range(
            start=last_known_datetime_for_pred_start + pd.Timedelta(hours=1), 
        ...
        for i in range(num_steps):
            ...
            # Prepare the features for the *next* input sequence
            # 1. Create a new row with the current prediction's target values and the next timestamp
            new_predicted_target_row_dict = {name: pred_step_all_targets_inversed[j] for j, name in enumerate(self.target_names)}
            new_predicted_target_row_dict['datetime'] = future_timestamps[i] # This is the timestamp for which we just predicted
            ...
            # 2. Append this new row to our history_df (which contains target_names and datetime info)
            ...
            # The window size should be enough for sequence_length + max_lag/roll_window
            max_lookback_for_features = self.sequence_length + max(self.lag_steps, max(self.rolling_windows) if self.rolling_windows else 0, 0)
            history_df_for_features = history_df_for_features.iloc[-max_lookback_for_features:].copy()


            # 3. From this updated history_df, take the last `sequence_length` rows to form the basis for the next input's features
            df_for_next_input_features = history_df_for_features.iloc[-self.sequence_length:].copy()

            # 4. Recalculate all features for these `sequence_length` rows
            df_for_next_input_features = self._add_time_features(df_for_next_input_features)
            df_for_next_input_features = self._add_lagged_features(df_for_next_input_features, self.target_names, self.lag_steps)
            cols_for_rolling = ['PM2.5', 'PM10', 'O3', 'AQI']
            df_for_next_input_features = self._add_rolling_features(df_for_next_input_features, [c for c in cols_for_rolling if c in df_for_next_input_features.columns], self.rolling_windows)
            
            # 5. Handle any NaNs created in the new features (especially at the start of the rolled sequence)
            # For the *input* to the model, we need a full sequence_length of valid features.
            # The last row of df_for_next_input_features contains the features for the *next* time step.
            # We need to ensure all rows in the *sequence* used for input are valid.
            # A simpler approach for now: take the last row's features, scale them, and update the input sequence.
            # This means that for the *next* prediction, the lagged/rolled features within the *input sequence*
            # are based on the *previous* iteration's state of history_df. This is a slight simplification.
            
            # Get the features for the *last row* of the re-calculated df_for_next_input_features
            # This last row corresponds to the current prediction time step future_timestamps[i]
            next_input_features_unscaled_vector = df_for_next_input_features[self.feature_names].iloc[-1].values
            
            # If NaNs are present in this vector (e.g. std of a single point), fill them
            # This can happen if rolling window is larger than available new data points in history_df
            if pd.isna(next_input_features_unscaled_vector).any():
                # print(f"NaNs found in next_input_features_unscaled_vector for step {i+1}, attempting ffill/bfill.")
                temp_feature_df = pd.DataFrame([next_input_features_unscaled_vector], columns=self.feature_names)
                temp_feature_df = temp_feature_df.ffill().bfill()
                next_input_features_unscaled_vector = temp_feature_df.iloc[0].values
                if pd.isna(next_input_features_unscaled_vector).any():
                    print(f"CRITICAL WARNING: NaNs still present in next_input_features after fill for step {i+1}. This will likely cause issues.")
                    # Fallback: use previous step's features for these NaNs, or a global mean, or zero.
                    # For simplicity, let's use the corresponding feature from the previous valid input vector.
                    # This is not ideal but prevents immediate crash.
                    prev_valid_features = current_input_sequence_scaled[-1, :] # Scaled
                    nan_indices = np.where(pd.isna(next_input_features_unscaled_vector))[0]
                    # We need to scale prev_valid_features back, replace NaN, then re-scale.
                    # Or, simpler: just use the previous scaled value for that feature index.
                    for nan_idx in nan_indices:
                         next_input_features_unscaled_vector[nan_idx] = self.scaler_X.inverse_transform(current_input_sequence_scaled[-1,:].reshape(1,-1))[0,nan_idx] # Risky if scaler changes much
