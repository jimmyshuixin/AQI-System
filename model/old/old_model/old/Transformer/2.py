...
class AQIPredictor:
    def __init__(self, data_path, model_path="trained_model.h5"):
        ...
        self._load_and_prepare_data() # 加载主训练/验证数据
    ...
    def _load_and_prepare_data(self, file_path=None, is_external_test_set=False):
        ...
        if self.date_col_name not in original_cols:
            raise ValueError(f"数据文件 {path_to_load} 中缺少日期列: {self.date_col_name}。")
        if self.hour_col_name not in original_cols:
            raise ValueError(f"数据文件 {path_to_load} 中缺少小时列: {self.hour_col_name}。")
        ...
        try:
            ...
            if not is_external_test_set: # 主数据加载
        ...
        if nan_counts_before.sum() > 0:
            ...
            if nan_counts_after.sum() > 0:
                ...
                print(f"信息: 文件 {path_to_load} - 所有NaN值已通过ffill().bfill()成功插补。")
        else:
            print(f"信息: 文件 {path_to_load} - 指标列中没有检测到NaN值。")
        
        if aqi_data.empty:
            raise ValueError(f"数据文件 {path_to_load} 在插补NaN值后为空。")
        
        if not is_external_test_set: # 主数据加载
            self.last_datetime_for_autoregression_start = aqi_data['datetime'].iloc[-1]
            print(f"信息: 主数据集中用于启动自回归预测的最后一个实际观测时间点为: {self.last_datetime_for_autoregression_start}")

        X_df = aqi_data[self.feature_names] 
        y_df = aqi_data[self.target_names]  

        X_values = X_df.iloc[:-1].values 
        y_values = y_df.iloc[1:].values  
        
        min_len = min(len(X_values), len(y_values))
        if min_len == 0:
            raise ValueError(f"文件 {path_to_load} 处理后没有足够的序列数据 (长度为0)。")
        ...
        if not is_external_test_set: # 主数据加载和训练集准备
            self.X_scaled_all_for_training = self.scaler_X.fit_transform(X_values)
            self.y_scaled = self.scaler_y.fit_transform(y_values) 
            self.X_input_for_future_pred_init_scaled = self.scaler_X.transform(aqi_data[self.feature_names].iloc[[-1]].values)

            if np.isnan(self.X_scaled_all_for_training).any(): print("警告: 特征数据 X_scaled_all_for_training 在归一化后包含NaN值。")
            if np.isnan(self.y_scaled).any(): print("警告: 目标数据 y_scaled 在归一化后包含NaN值。")
            ...
            self.y_test = self.y_scaled[train_size:] 
            ...
            X_ext_test_scaled = self.scaler_X.transform(X_values)
            y_ext_test_scaled = self.scaler_y.transform(y_values) 
            
            X_ext_test_reshaped = X_ext_test_scaled.reshape((X_ext_test_scaled.shape[0], 1, X_ext_test_scaled.shape[1]))
            
            ext_test_dates = datetime_for_y.reset_index(drop=True)
            return X_ext_test_reshaped, y_ext_test_scaled, ext_test_dates
    ...
    def train_and_evaluate(self, model, epochs=50, X_train_override=None, y_train_override=None, X_test_override=None, y_test_override=None):
        """
        训练模型并评估在测试集上的表现。
        Allows overriding train/test sets for specific evaluation scenarios.
        """
        ...
        y_te_scaled = y_test_override if y_test_override is not None else self.y_test # This is scaled y
        ...
        y_test_inversed = self.scaler_y.inverse_transform(y_te_scaled) # Use the correct y_test (scaled)
    ...
    def tune_model(self):
        ...
        try:
            def build_tuned_model(hp):
                ...
                dropout_rate = hp.Float('dropout', min_value=0.05, max_value=0.4, step=0.05) 
            ...
            tuner = BayesianOptimization(
                ...
                project_name='aqi_transformer_optimization_v2', 
                overwrite=True 
            )
    ...
    def predict_future(self, model, city_name, output_dir, num_steps=72):
        ...
        current_input_X_scaled = self.X_input_for_future_pred_init_scaled.copy() 
...
def select_reference_file_for_output(title="选择参考文件 (用于输出目录和城市名)"): 
    root = tk.Tk(); root.withdraw(); path = filedialog.askopenfilename(title=title, filetypes=[("Excel", "*.xlsx"),("CSV", "*.csv"),("All", "*.*")]); root.destroy(); return path
