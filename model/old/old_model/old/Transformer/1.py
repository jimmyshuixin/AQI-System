...
class AQIPredictor:
    ...
    def _load_and_prepare_data(self):
        ...
        # Transformer模型通常期望输入是 (batch_size, sequence_length, feature_dim)
        # 当前设置中，sequence_length=1, feature_dim=num_input_features (14)
    ...
    def _transformer_encoder_block(self, inputs, d_model, num_heads, ff_dim, dropout_rate):
        ...
        )(inputs, inputs) # Query, Value, Key are all 'inputs' for self-attention
        # Add & Norm
    ...
    def build_model(self, d_model=64, num_transformer_blocks=1, num_heads=4, ff_dim=128, dropout_rate=0.1, learning_rate=0.001):
        ...
        """
        # Input shape: (batch_size, 1, num_input_features)
        # num_input_features is self.X_train.shape[2]
        inputs = Input(shape=(self.X_train.shape[1], self.X_train.shape[2])) 

        # 1. Initial Projection/Embedding (optional, but good for Transformer)
        # Project input features (14) to d_model
        x = Dense(d_model, activation="relu")(inputs) # Output: (batch_size, 1, d_model)
        ...
        # 3. Output Layer
        # The output of Transformer block is (batch_size, 1, d_model)
        # Flatten it before the final Dense layer
        x = Flatten()(x) # Output: (batch_size, d_model)
        outputs = Dense(len(self.target_names))(x) # Output: (batch_size, num_targets=15)
    ...
    def load_existing_model(self):
        ...
        try:
            # 加载模型时，如果包含自定义层（如Transformer块），可能需要 custom_objects
            # 但这里Transformer块是用标准Keras层构建的，所以通常不需要
    ...
    def tune_model(self):
        ...
        try:
            def build_tuned_model(hp):
                ...
                # 确保 key_dim * num_heads = d_model, 或者 d_model % num_heads == 0
                # MultiHeadAttention 会自动处理 key_dim 如果未提供，只要 d_model 能被 num_heads 整除
                # 如果 d_model 不能被 num_heads 整除，MultiHeadAttention 会报错
                # 为简单起见，这里不动态调整，选择的d_model和num_heads应能兼容
                # 或者在 MultiHeadAttention 中设置 key_dim = d_model // num_heads
                
                ff_dim_factor = hp.Choice('ff_dim_factor', [2, 4])
                ff_dim = d_model * ff_dim_factor
                
                dropout_rate = hp.Float('dropout', min_value=0.1, max_value=0.4, step=0.05) 
            ...
            tuner = BayesianOptimization(
                build_tuned_model,
                objective='val_loss',
                max_trials=15, # 增加试验次数
                executions_per_trial=1, 
                directory=tuner_base_dir, 
                project_name='aqi_transformer_optimization', 
                overwrite=True 
            )
            ...
            if not best_hp_list:
                ...
                return self.build_model() # 使用默认参数
...
def main(): 
    ...
    model_path_selected = select_model_file_path("请指定模型文件的加载/保存路径", f"{city_name_for_forecast}_transformer_model.keras") 
    ...
    if train_model_choice:
        ...
        if model_to_use: 
            ...
            optimizer = keras.optimizers.Adam(learning_rate=1e-5) # 更小的学习率进行微调
            model_to_use.compile(optimizer=optimizer, loss='mean_squared_error', metrics=['mae'])
            baseline_predictions, y_test_inv_train = predictor.train_and_evaluate(model_to_use, epochs=30) # 微调epochs
        else: 
            print("构建并训练新的基线模型...")
            model_to_use = predictor.build_model(d_model=128, num_transformer_blocks=2, num_heads=4, ff_dim=256, learning_rate=0.001)
            baseline_predictions, y_test_inv_train = predictor.train_and_evaluate(model_to_use, epochs=150) # 初始训练epochs
        ...
        # 无论是否自动触发，如果用户选择了训练，都应该有机会进行优化
        # 或者，如果自动触发了，就直接进行优化
        run_optimization_flow = needs_auto_retrain_opt
        if not needs_auto_retrain_opt: # 如果没自动触发，问用户
            root = tk.Tk()
            root.withdraw()
            run_user_optimization = messagebox.askyesno("模型优化", "基线模型已训练/加载。\n您想运行贝叶斯优化以尝试改进模型吗？\n（这可能需要一些时间）")
            root.destroy()
            if run_user_optimization:
                run_optimization_flow = True
        
        if run_optimization_flow:
            ...
            if optimized_model_tuned:
                print("训练和评估优化后的模型...")
                # Tuner返回的模型应该是编译好的，但以防万一
                if not optimized_model_tuned.optimizer: 
                    lr_opt = optimized_model_tuned.get_config().get("optimizer_config", {}).get("learning_rate", 1e-3) # Keras 3.x
                    optimized_model_tuned.compile(optimizer=keras.optimizers.Adam(learning_rate=lr_opt), loss='mean_squared_error', metrics=['mae'])
                
                optimized_predictions, _ = predictor.train_and_evaluate(optimized_model_tuned, epochs=150) 
        ...
        else: # No optimization run (neither auto nor user-chosen)
            optimized_predictions = baseline_predictions
            eval_optimized = eval_baseline
            ...
            if predictor.X_test.size > 0:
                ...
                if needs_auto_retrain_opt_loaded:
                    ...
                    if optimized_model_tuned:
                        print("训练和评估自动优化后的模型...")
                        if not optimized_model_tuned.optimizer:
                            lr_opt = optimized_model_tuned.get_config().get("optimizer_config", {}).get("learning_rate", 1e-3)
                            optimized_model_tuned.compile(optimizer=keras.optimizers.Adam(learning_rate=lr_opt), loss='mean_squared_error', metrics=['mae'])
