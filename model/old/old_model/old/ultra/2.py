...
def setup_matplotlib_chinese_font():
    ...
    fonts_to_try = ['SimHei', 'Microsoft YaHei', 'Arial Unicode MS', 'sans-serif'] # 添加通用sans-serif作为最后选项
...
MSG_FILE_SELECT_SITE_INFO = "请选择站点信息CSV/Excel文件 (可选)"
...
MSG_CITY_NOT_FOUND_IN_SITES = "输入的城市/站点 '{}' 未在站点信息中找到。"
...
class AQICalculator:
    ...
    def calculate_aqi_from_df(df: pd.DataFrame) -> pd.DataFrame:
        df_copy = df.copy()
        iaqi_cols = []
        for p_base in TARGET_POLLUTANTS: # p_base 应该是 'PM2_5', 'SO2' 等
...
class AirQualityDataProcessor:
    """处理数据加载、预处理和特征工程。"""
    def __init__(self, sequence_length: int = 24, site_info_path: Optional[str] = None):
        self.sequence_length = sequence_length
        self.scaler = MinMaxScaler()
        self.site_to_id: Dict[str, int] = {}
        self.id_to_site: Dict[int, str] = {}
        self.num_sites = 0
        self.feature_columns: List[str] = []

        if site_info_path:
            self._load_site_mapping(site_info_path)

    def _load_site_mapping(self, site_info_path: str):
        site_df = None
        try:
            if site_info_path.endswith(('.xlsx', '.xls')):
                site_df = pd.read_excel(site_info_path)
                logging.info(f"成功读取站点信息 Excel 文件: {site_info_path}")
            elif site_info_path.endswith('.csv'):
                encodings_to_try = ['utf-8', 'gbk', 'latin1']
                for encoding in encodings_to_try:
                    try:
                        site_df = pd.read_csv(site_info_path, encoding=encoding)
                        logging.info(f"成功使用编码 {encoding} 读取站点信息 CSV 文件。")
                        break
                    except UnicodeDecodeError:
                        logging.warning(f"使用编码 {encoding} 读取站点信息 CSV 文件失败。")
                if site_df is None:
                    logging.error(f"尝试所有编码后仍无法读取站点信息 CSV 文件: {site_info_path}")
                    return
            else:
                logging.error(f"不支持的站点信息文件格式: {site_info_path}。请使用CSV或Excel。")
                return
        except FileNotFoundError:
            logging.error(f"站点信息文件未找到: {site_info_path}")
            return
        except Exception as e:
            logging.error(f"加载站点信息文件时发生未知错误: {e}")
            return
        
        if site_df is None or site_df.empty:
            logging.warning(f"站点信息文件 {site_info_path} 为空或读取失败。")
            return

        site_name_col = next((col for col in ['SiteName', 'city', 'station', '站点名称', site_df.columns[0]] if col in site_df.columns), None)
        site_id_col = next((col for col in ['SiteID_Encoded', 'SiteID', 'id', '站点ID', site_df.columns[1]] if len(site_df.columns) > 1 and col in site_df.columns), None)
        
        if not site_name_col:
            logging.error(f"无法在站点信息文件中确定站点名称列。可用列: {site_df.columns.tolist()}")
            return
        if self.num_sites > 0 and not site_id_col : # 如果期望有ID但没找到
             logging.warning(f"站点信息文件似乎只有一列，假设其为站点名称，SiteID将自动生成。")


        for _, row in site_df.iterrows():
            try:
                site_name = str(row[site_name_col])
                # 如果没有明确的ID列，或者ID列无效，可以考虑使用城市名本身或其哈希作为ID，或者自增ID
                site_id_val = None
                if site_id_col and pd.notna(row[site_id_col]):
                    site_id_val = int(row[site_id_col])
                else: # 自动生成ID
                    if site_name not in self.site_to_id: # 确保每个名称只有一个ID
                        site_id_val = len(self.site_to_id) # 简单的自增ID
                
                if site_id_val is not None:
                    self.site_to_id[site_name] = site_id_val
                    self.id_to_site[site_id_val] = site_name
            except (ValueError, TypeError) as e:
                logging.warning(f"跳过无效的站点信息行: {row}, 错误: {e}")
                continue
        self.num_sites = len(self.site_to_id)
        logging.info(f"从 {site_info_path} 加载/生成了 {self.num_sites} 个站点信息。")
    ...
    def _add_site_id_feature(self, df: pd.DataFrame, target_city_name: str) -> pd.DataFrame:
        if not self.site_to_id:
            logging.debug("站点映射未加载。SiteID 特征将不会被添加。") # 改为debug，因为城市特定模型中SiteID不那么关键
            return df
        site_id_for_city = self.site_to_id.get(target_city_name)
        if site_id_for_city is not None: df[SITE_ID_FEATURE] = site_id_for_city
        else:
            df[SITE_ID_FEATURE] = -1 
            logging.debug(f"城市 '{target_city_name}' 在站点信息中未找到对应SiteID。SiteID将设为-1。")
        return df

    def load_and_preprocess_data(self, data_root_folder_path: str, 
                                 target_city_to_filter: str, 
                                 raw_city_column_name_in_csv: str) -> Optional[pd.DataFrame]:
        ...
        standardized_cols = {}
        for col in df.columns: # 标准化透视后的污染物列名
            std_col_name = str(col).replace('.', '_') # 将 'PM2.5' 转为 'PM2_5'
            standardized_cols[col] = std_col_name
        df.rename(columns=standardized_cols, inplace=True)
        
        for p in TARGET_POLLUTANTS: # TARGET_POLLUTANTS 应该是 'PM2_5' 格式
        ...
        except ValueError as ve: # _add_time_features 可能会因无法创建索引而抛出错误
        ...
        if self.site_to_id: df = self._add_site_id_feature(df, target_city_to_filter)
        ...
        if SITE_ID_FEATURE in df.columns: self.feature_columns.append(SITE_ID_FEATURE)
    ...
    def create_sequences(self, data: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if list(data.columns) != self.feature_columns:
            ...
            try:
                # 确保只选择 feature_columns 中存在的列，并按其顺序
                cols_present_in_data = [col for col in self.feature_columns if col in data.columns]
                if len(cols_present_in_data) != len(self.feature_columns):
                    logging.warning(f"并非所有期望的特征列 ({self.feature_columns}) 都存在于数据中 ({data.columns.tolist()})。")
                data_for_sequence = data[cols_present_in_data] # 使用实际存在的、按序的特征列
                # 更新 self.feature_columns 以反映实际用于序列创建的列，这很重要
                # 但这里不应修改类的 self.feature_columns，而应在调用处确保一致性
                # 或者，create_sequences 应该严格使用传入的 data 的列
                current_feature_cols_for_sequence = list(data_for_sequence.columns)

            except KeyError as e:
                logging.error(f"创建序列时，数据中缺少必要的特征列: {e}")
                raise ValueError(f"创建序列时，数据中缺少必要的特征列: {e}")
        else:
             current_feature_cols_for_sequence = self.feature_columns
...
class AirQualityModel:
    """管理Keras模型。"""
    def __init__(self, sequence_length: int, num_features: int, num_target_pollutants: int, num_sites: int = 0):
        self.sequence_length = sequence_length
    ...
    def build_model(self, hp: Optional[HyperParameters] = None) -> Model:
        ...
        output = Dense(self.num_target_pollutants, activation="linear")(x) # 确保输出维度正确
    ...
    def tune_model(self, X_train: np.ndarray, y_train: np.ndarray, X_val: np.ndarray, y_val: np.ndarray,
        ...
        tuner = BayesianOptimization(
            ...
            executions_per_trial=1, directory=tuner_dir, project_name=project_name, overwrite=True) # overwrite=True
    ...
    def predict_future_steps(self, last_sequence: np.ndarray, num_steps: int) -> np.ndarray:
        ...
        # target_indices_in_features 应该基于模型训练时确定的 AirQualityPredictor.processor.feature_columns
        # 和 TARGET_POLLUTANTS
        ...
        for _ in range(num_steps):
            ...
            # 确保 next_step_pred_scaled (模型输出) 的数量与我们能找到的 target_indices_in_features 数量匹配
            # 或者至少，模型输出的数量 (self.num_target_pollutants) 应该与 target_indices_in_features 的预期数量一致
            if len(next_step_pred_scaled) == self.num_target_pollutants and self.num_target_pollutants == len(target_indices_in_features):
                ...
                # 尝试用模型输出的前N个值更新特征的前N个目标索引（如果数量匹配）
...
class PlottingUtils:
    ...
    def plot_predictions_vs_actual(y_true: np.ndarray, y_pred: np.ndarray, pollutant_names: List[str],
        ...
        for i in range(num_pollutants):
            # pollutant_names 应该与 y_true/y_pred 的列对应
            # 如果 pollutant_names 来自 TARGET_POLLUTANTS，而 y_pred 的列顺序可能不同，这里会有问题
            # 假设 y_pred 的列顺序与 TARGET_POLLUTANTS 对应
...
class AirQualityPredictor:
    ...
    def __init__(self, sequence_length: int = 24, 
                 city_specific_output_dir: str = "city_results",
                 site_info_path: Optional[str] = None):
        self.sequence_length = sequence_length
        self.output_dir = city_specific_output_dir
        os.makedirs(self.output_dir, exist_ok=True)

        AirQualityPredictor.processor = AirQualityDataProcessor(sequence_length, site_info_path)
    ...
    def _prepare_data_for_model(self, data_root_folder_path: str, 
                                ...
                                raw_city_column_name_in_csv: str,
        ...
        try:
            self.data_scaled = AirQualityPredictor.processor.load_and_preprocess_data(
                data_root_folder_path, target_city_name, raw_city_column_name_in_csv
            )
        except (FileNotFoundError, ValueError) as e: # load_and_preprocess_data 内部会messagebox
            logging.error(f"为城市 '{target_city_name}' 加载或预处理数据失败: {e}")
            return False # 阻止进一步执行

        if self.data_scaled is None or self.data_scaled.empty:
            return False

        n = len(self.data_scaled)
        min_data_needed = (self.sequence_length + 1) * 3 # 至少需要训练、验证、测试各一个序列
        ...
        if num_target_pollutants == 0: # 关键检查
        ...
        self.model_pipeline = AirQualityModel(
            self.sequence_length, num_features, num_target_pollutants,
            AirQualityPredictor.processor.num_sites
        )
        return True
    ...
    def train_evaluate_flow(self, data_root_folder_path: str,
        ...
        if not self._prepare_data_for_model(data_root_folder_path, target_city_name, raw_city_column_name_in_csv):
            return None, None, False
        if not self.model_pipeline: # _prepare_data_for_model 内部会创建
        ...
        if tune_model_flag:
            ...
            try:
                ...
                if not self.model_pipeline.model: self.model_pipeline.build_model() # 确保有模型
        else:
            logging.info(f"跳过城市 '{target_city_name}' 的超参数调优，使用默认参数。")
            self.model_pipeline.build_model()

        if not self.model_pipeline.model : # 如果build_model或tune_model后模型仍为空
        ...
        if self.X_test.size > 0 and self.y_test.size > 0:
            ...
            # 确保 y_pred_final 的列数与 TARGET_POLLUTANTS 匹配，或使用 y_test.shape[1] 来确定 pollutant_names
            # 假设 TARGET_POLLUTANTS 是正确的，并且 y_pred_final 的列与之对应
    ...
    def predict_future(self, city_name_to_predict: str, 
                       ...
                       raw_city_column_name_in_csv_for_context: str,
                       num_steps: int = 72):
        
        if not AirQualityPredictor.processor:
            # 为预测场景初始化一个临时的processor，如果之前没有训练过
            # site_info_path 可能需要从GUI获取，或假设预测时不需要站点ID特征
            site_info_path_gui = self.site_info_file_path_var.get() if hasattr(self, 'site_info_file_path_var') else None
            AirQualityPredictor.processor = AirQualityDataProcessor(self.sequence_length, site_info_path_gui)
        ...
        if not self.model_pipeline or not self.model_pipeline.model or self.trained_city_name != city_name_to_predict:
            if os.path.exists(model_load_path):
                ...
                try:
                    ...
                    self.model_pipeline = AirQualityModel(_seq_len, _num_features, _num_targets, AirQualityPredictor.processor.num_sites)
        ...
        if not self.model_pipeline or not self.model_pipeline.model: # 双重检查
        ...
        try:
            city_data_scaled = AirQualityPredictor.processor.load_and_preprocess_data(
                ...
                city_name_to_predict, 
                raw_city_column_name_in_csv_for_context
            )
        ...
        # 确保 processor.feature_columns 与模型期望的 num_features 一致
        ...
        try:
            # 使用 processor 中最新的 feature_columns 来提取序列
        ...
        # 反归一化时使用的 target_indices 也应基于 processor 最新的 feature_columns
        ...
        for tp in TARGET_POLLUTANTS:
            if tp in AirQualityPredictor.processor.feature_columns:
                ...
                logging.debug(f"目标污染物 '{tp}' 未在 processor.feature_columns 中，反归一化时将跳过。")
        ...
        if predictions_scaled.shape[1] != len(target_indices_for_inverse):
            logging.warning(f"预测输出的污染物数量 ({predictions_scaled.shape[1]}) 与期望反归一化的数量 ({len(target_indices_for_inverse)}) 不符。将尝试匹配。")
            ...
            # 更新实际用于绘图和保存的污染物名称列表
            actual_pollutant_names_for_plot_save = [current_pollutant_names_for_inverse[i] for i in range(min_cols)]
            ...
            actual_pollutant_names_for_plot_save = current_pollutant_names_for_inverse
        ...
        PlottingUtils.plot_future_forecast(
            actual_predictions_inversed_subset, actual_pollutant_names_for_plot_save, city_forecast_output_dir, city_name_to_predict
        )
        pred_df = pd.DataFrame(actual_predictions_inversed_subset, columns=actual_pollutant_names_for_plot_save)
        ...
        pred_df.to_csv(os.path.join(city_forecast_output_dir, f"{city_name_to_predict}_future_predictions.csv"))
        logging.info(f"{city_name_to_predict} 的未来预测已生成并保存在 {city_forecast_output_dir}")
        messagebox.showinfo(MSG_INFO_TITLE, f"{city_name_to_predict} 的未来预测已生成并保存在 {city_forecast_output_dir}")
...
class AirQualityAppGUI:
    def __init__(self, root_window):
        ...
        self.site_info_file_path_var = tk.StringVar()
        ...
        self.city_column_in_csv_var = tk.StringVar(value="city") 
    ...
    def _select_file(self, string_var: tk.StringVar, title: str, is_model_file: bool = False):
        filetypes = [("Keras 模型", "*.keras *.h5")] if is_model_file else \
                    [("Excel 文件", "*.xlsx *.xls"),("CSV 文件", "*.csv"),  ("所有文件", "*.*")] # Excel优先
    ...
    def _setup_ui(self):
        ...
        tk.Button(file_path_frame, text="选择基础输出文件夹", command=lambda: self._select_dir(self.base_output_dir_path_var, MSG_DIR_SELECT_OUTPUT_BASE)).grid(row=2, column=0, sticky=tk.W, padx=5, pady=2)
        tk.Entry(file_path_frame, textvariable=self.base_output_dir_path_var, width=50).grid(row=2, column=1, padx=5, pady=2, sticky=tk.EW)
        ...
        tk.Entry(city_train_frame, textvariable=self.target_city_for_training_var, width=15).grid(row=0, column=1, sticky=tk.W, padx=5, pady=2)
        tk.Label(city_train_frame, text="CSV中城市列名(已弃用):").grid(row=0, column=2, sticky=tk.W, padx=(10,0), pady=2) # 提示用户此列名用途已改变
        tk.Entry(city_train_frame, textvariable=self.city_column_in_csv_var, width=15, state=tk.DISABLED).grid(row=0, column=3, sticky=tk.W, padx=5, pady=2) # 禁用
    ...
    def run_training_pipeline(self):
        ...
        raw_csv_city_col = self.city_column_in_csv_var.get().strip() 
        ...
        try:
            self.predictor_instance = AirQualityPredictor(
                ...
                site_info_path=self.site_info_file_path_var.get() if self.site_info_file_path_var.get() else None
            )
            ...
            _, final_metrics, _ = self.predictor_instance.train_evaluate_flow(
                ...
                raw_city_column_name_in_csv=raw_csv_city_col, 
    ...
    def run_future_prediction(self):
        ...
        base_output_dir = self.base_output_dir_path_var.get()
        data_root_path = self.data_root_folder_path_var.get()
        raw_csv_city_col = self.city_column_in_csv_var.get().strip() # 仍然需要，用于数据上下文
        ...
        if not model_path_for_city: # 如果用户未指定，则按约定构建路径
        ...
        try:
            predictor_for_future = AirQualityPredictor(
                sequence_length=self.sequence_length_var.get(),
                city_specific_output_dir=city_specific_model_dir, # 输出到此城市的目录
                site_info_path=self.site_info_file_path_var.get() if self.site_info_file_path_var.get() else None
            )
            ...
            predictor_for_future.predict_future(
                ...
                raw_city_column_name_in_csv_for_context=raw_csv_city_col if raw_csv_city_col else "city", 
