...
class AQIPredictor:
    ...
    def train_and_evaluate(self, model, epochs=50):
        ...
        early_stopping = keras.callbacks.EarlyStopping(
            monitor='val_loss', 
            patience=10,  # 如果验证损失在10个周期内没有改善，则停止
            restore_best_weights=True, # 恢复到最佳权重
            verbose=1 
        )
        reduce_lr = keras.callbacks.ReduceLROnPlateau(
            monitor='val_loss', 
            factor=0.2, # 学习率将乘以的因子
            patience=5,  # 如果验证损失在5个周期内没有改善，则降低学习率
            min_lr=1e-6, # 学习率的下限
            verbose=1
        )
    ...
    def tune_model(self):
        ...
        try:
            ...
            early_stopping_tuner = keras.callbacks.EarlyStopping(
                monitor='val_loss', 
                patience=5, # Tuner中的patience可以短一些
                verbose=0,
                restore_best_weights=True
            )

            tuner.search(self.X_train, self.y_train, 
                         epochs=50, # Tuner内部的epochs
                         validation_split=0.2, 
                         verbose=1, # 显示Tuner的进度
                         callbacks=[early_stopping_tuner]) # 为Tuner的search添加EarlyStopping
            ...
            # Keras Tuner 返回的模型通常是已编译的，但以防万一
    ...
    def predict_future(self, model, city_name, output_dir, num_steps=72):
        ...
        print("注意: 自回归预测中，除AQI外的其他污染物指标也是基于模型迭代预测的，这可能导致误差随预测步长增加而累积。")
...
def main(): 
    ...
    if train_model_choice:
        ...
        if model_to_use: 
            ...
            baseline_predictions, y_test_inv_train = predictor.train_and_evaluate(model_to_use, epochs=10)
        else: 
            print("构建并训练新的基线模型...")
            model_to_use = predictor.build_model(activation_func='tanh', learning_rate=0.001)
            baseline_predictions, y_test_inv_train = predictor.train_and_evaluate(model_to_use, epochs=50)
        ...
        if needs_auto_retrain_opt:
            ...
            if optimized_model_tuned:
                ...
                # optimized_predictions 保持为NaN, eval_optimized 保持为NaN字典
        else: 
            root = tk.Tk()
            root.withdraw()
            ...
            # else: # 用户不选择优化，optimized_predictions 和 eval_optimized 保持为初始的NaN状态
                # pass 
