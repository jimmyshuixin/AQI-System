...
class AQIPredictor:
    ...
    def _load_and_prepare_data(self):
        ...
        try:
            # 确保 'date' 是字符串格式 YYYYMMDD，'hour' 是可以补零的数字或字符串
            ...
            print("请确保 'date' 列是 YYYYMMDD 格式，'hour' 列是 0-23 范围内的小时数。")
            raise
    ...
    def plot_results(self, y_true, y_pred_base, y_pred_opt):
        ...
        # elif not pd.api.types.is_numeric_dtype(self.test_dates): # 这部分逻辑可能不再需要，因为 test_dates 应该是 datetime
        #     try:
        #         plot_dates = pd.to_datetime(self.test_dates)
    ...
    def plot_metrics_comparison(self, eval_baseline, eval_optimized):
        ...
        for i, model_name in enumerate(df_metrics.index):
            for j, metric_name in enumerate(df_metrics.columns):
                ...
                # 调整文本位置和大小以避免重叠
                text_x_offset_base = 0.0 # 基础偏移
                if len(df_metrics.columns) > 2: # 如果指标较多，稍微分散
...
def main(): 
    data_path = input("请输入数据集路径 (例如 data/Nanjing_AQI_Data.xlsx): ")
    model_path_input = input("请输入要加载/保存的模型文件路径 (例如 models/trained_model.h5, 直接回车使用默认 trained_model.h5): ")
    
    if not model_path_input.strip():
    ...
    if optimized_model is None: 
        ...
        # predictor.save_model(optimized_model, "optimized_model.h5") # 可以选择保存优化后的模型
