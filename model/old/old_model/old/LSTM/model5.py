...
class AQIPredictor:
    ...
    def predict_future(self, model, num_steps=72):
        ...
        if self.true_absolute_last_datetime is None:
            ...
            # 作为回退，尝试使用 self.last_datetime_with_complete_features
        ...
        for i in range(num_steps):
            ...
            # 对于这个非自回归模型（不使用预测的AQI作为下一时间步的输入特征），
            # 我们将继续使用最后一个已知的 *实际* 特征集进行所有未来预测。
        ...
        for i, model_name in enumerate(df_metrics.index): 
            for j, metric_name in enumerate(df_metrics.columns): 
                ...
                if pd.notna(bar_height) and bar_height > 0 :
                    text_y_position = bar_height * 0.95 # 尝试放在柱子内部靠上的位置
                    va_align = 'top'
                    if bar_height < (0.1 * df_metrics[metric_name].abs().max()): # 如果柱子太矮
                        text_y_position = bar_height + 0.01 * df_metrics[metric_name].abs().max() # 放在柱子上方
                        va_align = 'bottom'
                elif pd.notna(bar_height) and bar_height < 0: # 对于负值 (例如R2)
                ...
                plt.text(
                    ...
                    ha='center',
                    va=va_align,
                    fontsize=8, # 稍微减小字体以适应
                    color='black',
                    bbox=dict(facecolor='white', alpha=0.5, pad=1, boxstyle='round,pad=0.2') # 添加背景框
                )
...
def main(): 
        ...
        if run_optimization:
            ...
            if optimized_model_tuned:
                ...
                if not optimized_model_tuned.optimizer:
                    print("编译优化后的模型...")
                    lr_opt = 1e-3 # Default if not found
                    if hasattr(optimized_model_tuned, 'optimizer') and hasattr(optimized_model_tuned.optimizer, 'learning_rate'):
                        ...
                        except: pass # Keep default if error
    ...
    if y_test_inv is None and predictor.y_test.size > 0:
        y_test_inv = predictor.scaler_y.inverse_transform(predictor.y_test)
    elif y_test_inv is None:
    ...
    print("\n--- 生成图表 ---")
    if baseline_predictions is not None and optimized_predictions is not None and y_test_inv is not None:
