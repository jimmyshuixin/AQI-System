...
class AQIPredictor:
    ...
    def build_model(self, units1=50, units2=None, dropout_rate=0.2, activation_func='tanh', learning_rate=0.001, num_lstm_layers=1):
        """
        创建并编译双向LSTM模型，支持多目标输出和可选的堆叠LSTM层。
        """
        inputs = Input(shape=(self.X_train.shape[1], self.X_train.shape[2])) 
        x = Bidirectional(LSTM(units=units1, activation=activation_func, return_sequences=(num_lstm_layers > 1)))(inputs)
        x = Dropout(dropout_rate)(x)
        
        if num_lstm_layers > 1 and units2 is not None:
            x = Bidirectional(LSTM(units=units2, activation=activation_func))(x)
            x = Dropout(dropout_rate)(x) # 可以在第二层LSTM后也加Dropout
    ...
    def train_and_evaluate(self, model, epochs=50):
        ...
        reduce_lr = keras.callbacks.ReduceLROnPlateau(
            monitor='val_loss', 
            factor=0.2, 
            patience=7,  # 增加patience
            min_lr=1e-7, # 调整min_lr
            verbose=1
        )
    ...
    def tune_model(self):
        ...
        try:
            def build_tuned_model(hp):
                ...
                units1 = hp.Int('units1', min_value=32, max_value=384, step=32) # 扩大范围
                units2 = None
                if num_lstm_layers == 2:
                    units2 = hp.Int('units2', min_value=32, max_value=256, step=32) # 第二层可以小一些

                dropout_rate = hp.Float('dropout', min_value=0.1, max_value=0.5, step=0.05) # 更细致的dropout
                learning_rate = hp.Float('learning_rate', min_value=1e-5, max_value=1e-2, sampling='LOG', default=1e-3) # 扩大范围
                activation_func = 'tanh' 

                model = self.build_model(units1=units1, units2=units2, 
            ...
            tuner = BayesianOptimization(
                build_tuned_model,
                objective='val_loss',
                max_trials=20, # 增加试验次数
                executions_per_trial=1, 
                directory=tuner_base_dir, 
                project_name='aqi_optimization_v3', 
                overwrite=True 
            )
            
            early_stopping_tuner = keras.callbacks.EarlyStopping(
                monitor='val_loss', 
                patience=7, # Tuner中的patience
                verbose=0,
                restore_best_weights=True
            )

            tuner.search(self.X_train, self.y_train, 
                         epochs=100, # Tuner内部的epochs可以多一些，由EarlyStopping控制
                         validation_split=0.2, 
                         verbose=1, 
            ...
            print("找到的最佳超参数:")
            print(f"Num LSTM Layers: {best_hp.get('num_lstm_layers')}")
            print(f"Units1: {best_hp.get('units1')}")
            if best_hp.get('num_lstm_layers') == 2:
    ...
    def predict_future(self, model, city_name, output_dir, num_steps=72):
        ...
        print("注意: 自回归预测中，所有指标（包括AQI和污染物）都是基于模型迭代预测的。这意味着模型使用其自身在前一时间步对所有15个指标的预测值，来形成下一时间步的14个污染物输入特征。这种方式可能导致误差随预测步长增加而累积。")
        ...
        for col in self.target_names:
            df_future[col] = df_future[col].round().astype(int)

        df_future['date'] = future_timestamps.strftime('%Y%m%d')
        df_future['hour'] = future_timestamps.hour
        
        final_forecast_columns = ['date', 'hour'] + self.target_names
        df_future = df_future[final_forecast_columns]
        ...
        try:
            df_future.to_excel(forecast_file_name, index=False)
        ...
        for i, target_col in enumerate(self.target_names):
            ax = axes[i]
            ax.plot(future_timestamps, df_future[target_col], label=f'预测 {target_col}', color='purple', marker='.', linestyle='-') 
    ...
    def evaluate(self, y_true_multi, y_pred_multi): 
        ...
        if num_targets != len(self.target_names):
            print(f"警告: 评估时目标数量 ({num_targets}) 与预期目标名称数量 ({len(self.target_names)}) 不匹配。")
            target_names_to_use = self.target_names[:num_targets]
        else:
            target_names_to_use = self.target_names
    ...
    def check_retraining_trigger(self, y_true_multi, y_pred_multi, deviation_threshold=0.20, points_threshold_percentage=0.30):
        """
        检查是否需要重新训练模型，基于所有预测指标的偏差。
        如果任何一个指标的偏差超过阈值，则返回True。
        """
        if y_true_multi is None or y_pred_multi is None or y_true_multi.shape[0] == 0:
            print("警告: 无法检查重训练触发条件，因为真实值或预测值为空。")
            return False
        
        if y_true_multi.shape[1] != len(self.target_names) or y_pred_multi.shape[1] != len(self.target_names):
            print("警告: 真实值或预测值的维度与目标数量不匹配，无法检查重训练触发条件。")
            return False

        triggered_for_retraining = False
        for i, target_name in enumerate(self.target_names):
            y_true_single = y_true_multi[:, i]
            y_pred_single = y_pred_multi[:, i]

            y_true_flat = np.array(y_true_single).flatten()
            y_pred_flat = np.array(y_pred_single).flatten()

            valid_mask = ~np.isnan(y_pred_flat) & ~np.isinf(y_pred_flat) & \
                         ~np.isnan(y_true_flat) & ~np.isinf(y_true_flat)
            
            if not np.any(valid_mask):
                print(f"警告: 指标 '{target_name}' 的真实值或预测值全部为NaN/inf，跳过此指标的偏差检查。")
                continue
                
            y_true_f = y_true_flat[valid_mask]
            y_pred_f = y_pred_flat[valid_mask]

            if len(y_true_f) == 0:
                print(f"警告: 指标 '{target_name}' 没有有效的真实值和预测值对，跳过此指标的偏差检查。")
                continue

            deviations = np.abs(y_pred_f - y_true_f) / (y_true_f + 1e-8) 
            points_exceeding_dev = np.sum(deviations > deviation_threshold)
            percentage_exceeding = points_exceeding_dev / len(y_true_f)

            print(f"信息: 指标 '{target_name}' 预测偏差检查 - {points_exceeding_dev} 个点 (共 {len(y_true_f)}个有效点) 的偏差超过 {deviation_threshold*100:.0f}%。")
            print(f"信息: 指标 '{target_name}' 偏差超过阈值的点所占百分比: {percentage_exceeding*100:.2f}% (触发阈值: {points_threshold_percentage*100:.0f}%)")

            if percentage_exceeding >= points_threshold_percentage:
                print(f"触发条件满足: 指标 '{target_name}' 的预测性能不佳。")
                triggered_for_retraining = True # 标记需要重训练，但继续检查其他指标
        
        if triggered_for_retraining:
            print("一个或多个指标的预测性能不佳，建议重新训练和优化。")
            return True
        else:
            print("所有指标的预测性能均在可接受范围内，无需自动重训练。")
            return False
        ...
        for i, metric_name in enumerate(metric_names_to_plot):
            ...
            def autolabel(rects, original_values_dict_for_metric):
                for k_rect, rect in enumerate(rects): 
                    ...
                    target_for_label = plot_target_labels[k_rect] 
...
def select_reference_file_for_output(title="请选择一个参考Excel文件 (用于确定输出目录和城市名)"):
    """打开文件对话框选择参考文件路径。"""
    root = tk.Tk()
    root.withdraw()
    file_path = filedialog.askopenfilename(
...
def main(): 
    ...
    data_path = select_data_file_path("请选择 AQI 数据集 Excel 文件 (用于训练/测试)")
    if not data_path:
        print("未选择数据集文件，程序退出。")
        return

    output_ref_file_path = None
    city_name_for_forecast = "DefaultCity" # 默认城市名
    forecast_output_dir = "." # 默认输出到当前目录

    if predict_future_choice: # 仅当用户选择预测未来时才要求参考文件
        output_ref_file_path = select_reference_file_for_output(
            "请选择一个参考文件 (例如，与训练数据同目录的任意文件，用于确定预测文件的保存位置和城市名)"
        )
        if not output_ref_file_path:
            print("未选择参考文件，未来预测结果将保存在当前目录，并使用默认城市名。")
        else:
            forecast_output_dir = os.path.dirname(output_ref_file_path)
            base_name = os.path.basename(output_ref_file_path)
            # 尝试从文件名提取城市名，例如 "南京_AQI_Data_test.xlsx" -> "南京"
            match = re.match(r"([^_]+)_AQI_Data", base_name, re.UNICODE)
            if match:
                city_name_for_forecast = match.group(1)
            else:
                # 如果模式不匹配，尝试取点之前的部分作为城市名
                city_name_for_forecast = os.path.splitext(base_name)[0].split('_')[0]
            print(f"未来预测将使用城市名: '{city_name_for_forecast}' 并保存在目录: '{forecast_output_dir}'")


    model_path_selected = select_model_file_path("请指定模型文件的加载/保存路径", f"{city_name_for_forecast}_multi_target_model.keras") 
    ...
    if predictor.y_test.size > 0 : 
        ...
        # 初始化 optimized_predictions 为与 y_test_inv 形状相同的NaN数组
        # 确保 y_test_inv 是 (samples, num_targets)
        if y_test_inv.ndim == 1: # 如果 y_test_inv 意外地是一维的
            if len(static_target_names) == 1:
                y_test_inv = y_test_inv.reshape(-1,1)
            else: # 如果目标多于一个，但y_test_inv是一维，则有问题
                print("错误: y_test_inv 维度与目标数量不匹配。")
                return
    ...
    if train_model_choice:
        ...
        if model_to_use: 
            ...
            model_to_use.compile(optimizer=optimizer, loss='mean_squared_error', metrics=['mae'])
            baseline_predictions, y_test_inv_train = predictor.train_and_evaluate(model_to_use, epochs=20) # 增加微调epochs
        else: 
            print("构建并训练新的基线模型...")
            model_to_use = predictor.build_model(activation_func='tanh', learning_rate=0.001, units1=128) # 尝试增加默认单元数
            baseline_predictions, y_test_inv_train = predictor.train_and_evaluate(model_to_use, epochs=100) # 增加初始训练epochs
        ...
        if y_test_inv is not None and baseline_predictions is not None:
            eval_baseline = predictor.evaluate(y_test_inv, baseline_predictions) 
        
        needs_auto_retrain_opt = predictor.check_retraining_trigger(
            y_test_inv, baseline_predictions 
        )

        if needs_auto_retrain_opt:
            print("\n--- 自动重新优化模型 ---")
            ...
            if optimized_model_tuned:
                ...
                if not optimized_model_tuned.optimizer: 
                    ...
                    if hasattr(optimized_model_tuned, 'optimizer') and hasattr(optimized_model_tuned.optimizer, 'learning_rate'):
                        try: lr_opt = optimized_model_tuned.optimizer.learning_rate.numpy()
                        except: pass
                    optimized_model_tuned.compile(optimizer=keras.optimizers.Adam(learning_rate=lr_opt), loss='mean_squared_error', metrics=['mae'])
                
                # 更新 optimized_predictions
                optimized_predictions, _ = predictor.train_and_evaluate(optimized_model_tuned, epochs=100) # 优化后也用较多epochs
                if y_test_inv is not None and optimized_predictions is not None:
                    eval_optimized = predictor.evaluate(y_test_inv, optimized_predictions)
                model_to_use = optimized_model_tuned 
                predictor.save_model(model_to_use) 
            else:
                print("自动优化失败。后续操作将使用之前训练的基线模型。")
                optimized_predictions = baseline_predictions # Fallback for plotting
                eval_optimized = eval_baseline # Fallback for eval
        else: 
            root = tk.Tk()
            root.withdraw()
            run_user_optimization = messagebox.askyesno("模型优化", "基线模型已训练/加载，且性能在可接受范围内。\n您仍想手动运行贝叶斯优化以尝试进一步改进模型吗？\n（这可能需要一些时间）")
            root.destroy()
            if run_user_optimization:
                ...
                if optimized_model_tuned:
                    ...
                    # 更新 optimized_predictions
                    optimized_predictions, _ = predictor.train_and_evaluate(optimized_model_tuned, epochs=100)
                    if y_test_inv is not None and optimized_predictions is not None:
                        eval_optimized = predictor.evaluate(y_test_inv, optimized_predictions)
                    model_to_use = optimized_model_tuned
                    predictor.save_model(model_to_use) 
                    ...
                    optimized_predictions = baseline_predictions # Fallback for plotting
                    eval_optimized = eval_baseline # Fallback for eval
            else: # 用户不选择优化，optimized_predictions 和 eval_optimized 保持与 baseline 一致
                optimized_predictions = baseline_predictions
                eval_optimized = eval_baseline
        ...
        if model_to_use:
            ...
            if predictor.X_test.size > 0:
                ...
                needs_auto_retrain_opt_loaded = predictor.check_retraining_trigger(
                    y_test_inv, baseline_predictions
                )
                if needs_auto_retrain_opt_loaded:
                    print("\n--- 自动重新优化已加载的模型 ---")
                    optimized_model_tuned = predictor.tune_model()
                    if optimized_model_tuned:
                        print("训练和评估自动优化后的模型...")
                        ...
                        optimized_predictions = baseline_predictions # Fallback for plotting
                        eval_optimized = eval_baseline # Fallback for eval
                else: # No auto-retrain needed for loaded model
                    optimized_predictions = baseline_predictions
                    eval_optimized = eval_baseline
            else:
    ...
    if predict_future_choice:
        if model_to_use:
            print("\n--- 未来所有指标预测 ---")
            predictor.predict_future(model_to_use, city_name=city_name_for_forecast, output_dir=forecast_output_dir, num_steps=72)
        else:
