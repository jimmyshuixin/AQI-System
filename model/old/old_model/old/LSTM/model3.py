...
class AQIPredictor:
    ...
    def tune_model(self):
        ...
        try:
            ...
            tuner = BayesianOptimization(
                ...
                directory=tuner_base_dir, # 使用临时目录
                project_name='aqi_optimization', # 项目名称
                overwrite=True # 如果项目存在于临时目录中，则覆盖
            )
    ...
    def evaluate(y_true, y_pred):
        ...
        # 处理y_pred中可能存在的NaN值，以避免评估函数报错
        mask = ~np.isnan(y_pred)
        if not np.any(mask): # 如果所有预测都是NaN
        ...
        if len(y_true_filtered) == 0: # 如果过滤后没有有效数据
    ...
    def plot_metrics_comparison(self, eval_baseline, eval_optimized):
        ...
        for model_type, metrics in metrics_data.items():
            plot_data[model_type] = {k: (metrics[k] if pd.notna(metrics[k]) else 0) for k in metric_names} # 用0替换NaN以便绘图，但标签会显示真实值或NaN
        ...
        for i, model_name in enumerate(df_metrics.index): # Baseline, Optimized
            for j, metric_name in enumerate(df_metrics.columns): # MAPE, RMSE, ...
                original_value = metrics_data[model_name][metric_name] # 从原始数据获取值 (可能为NaN)
                display_value_text = f"{original_value:.3f}" if pd.notna(original_value) else "N/A"
                
                bar_height = df_metrics.loc[model_name, metric_name] # 这是用于绘图的高度 (NaN被替换为0)
                ...
                plt.text(
                    ...
                    bar_height + 0.01 * df_metrics[metric_name].abs().max(), # 基于绘图高度定位
                    display_value_text, 
                    ha='center',
                    va='bottom',
                    fontsize=8 
...
def main(): 
    ...
    print("\n--- 优化模型 (使用贝叶斯优化) ---")
    optimized_model = predictor.tune_model() # tune_model 现在包含 try-except-finally
    
    if optimized_model is None or isinstance(optimized_model, str) and optimized_model == "Tuning_Error": # 假设出错时返回特定标记
    ...
    if optimized_model is not None and not (isinstance(optimized_model, str) and optimized_model == "Tuning_Error"):
