...
class AQIPredictor:
    ...
    def predict_future(self, model, num_steps=72, forecast_file_name="南京_AQI_Data_forecast.xlsx"):
        """
        ...
        for i, target_col in enumerate(self.target_names):
            ax = axes[i]
            ax.plot(future_timestamps, df_future[target_col], label=f'预测 {target_col}', color='purple', marker='.', linestyle='-') # 使用 future_timestamps
    ...
    def evaluate(self, y_true_multi, y_pred_multi): # 修改为实例方法
        """
        计算多目标输出的评价指标。
    ...
    def check_retraining_trigger(self, y_true_aqi, y_pred_aqi, deviation_threshold=0.20, points_threshold_percentage=0.30):
        """
        检查是否需要重新训练模型，基于AQI预测的偏差。
        """
        if y_true_aqi is None or y_pred_aqi is None or len(y_true_aqi) == 0:
            print("警告: 无法检查重训练触发条件，因为真实值或预测值为空。")
            return False

        y_true_flat = np.array(y_true_aqi).flatten()
        y_pred_flat = np.array(y_pred_aqi).flatten()

        valid_mask = ~np.isnan(y_pred_flat) & ~np.isinf(y_pred_flat) & \
                     ~np.isnan(y_true_flat) & ~np.isinf(y_true_flat)
        
        if not np.any(valid_mask):
            print("警告: AQI的真实值或预测值全部为NaN/inf，无法检查重训练触发条件。")
            return False
        ...
        if len(y_true_f) == 0:
            print("警告: AQI没有有效的真实值和预测值对，无法检查重训练触发条件。")
            return False

        deviations = np.abs(y_pred_f - y_true_f) / (y_true_f + 1e-8) # Add epsilon to avoid division by zero
        points_exceeding_dev = np.sum(deviations > deviation_threshold)
        percentage_exceeding = points_exceeding_dev / len(y_true_f)

        print(f"信息: AQI预测偏差检查 - {points_exceeding_dev} 个点 (共 {len(y_true_f)}个有效点) 的偏差超过 {deviation_threshold*100:.0f}%。")
        print(f"信息: 偏差超过阈值的点所占百分比: {percentage_exceeding*100:.2f}% (触发阈值: {points_threshold_percentage*100:.0f}%)")

        if percentage_exceeding >= points_threshold_percentage:
            print("触发条件满足: 模型的AQI预测性能不佳，建议重新训练和优化。")
            return True
        return False
...
def main(): 
    ...
    static_target_names = predictor.target_names # 使用实例的target_names
    ...
    if train_model_choice:
        print("\n--- 模型训练流程 ---")
        model_to_use = predictor.load_existing_model() # 尝试加载
        
        if model_to_use: # 如果加载成功，作为基线进行微调
        ...
        predictor.save_model(model_to_use) # 保存当前最佳模型 (可能是微调的或新训练的)
        
        if y_test_inv is not None and baseline_predictions is not None:
            eval_baseline = predictor.evaluate(y_test_inv, baseline_predictions) # 注意这里是实例方法
        
        # 检查是否需要自动重训练和优化
        aqi_idx = predictor.target_names.index('AQI')
        needs_auto_retrain_opt = predictor.check_retraining_trigger(
            y_test_inv[:, aqi_idx] if y_test_inv is not None else None,
            baseline_predictions[:, aqi_idx] if baseline_predictions is not None else None
        )
        ...
        if needs_auto_retrain_opt:
            ...
            if optimized_model_tuned:
                print("训练和评估自动优化后的模型...")
                if not optimized_model_tuned.optimizer: # 确保编译
                    lr_opt = optimized_model_tuned.get_config().get("optimizer", {}).get("learning_rate", 1e-3)
                ...
                model_to_use = optimized_model_tuned # 更新为优化模型
                predictor.save_model(model_to_use) # 保存优化后的模型
            else:
                print("自动优化失败。后续操作将使用之前训练的基线模型。")
                # optimized_predictions 和 eval_optimized 保持为初始的NaN状态
        else: # 如果不需要自动优化，再询问用户是否手动优化
            root = tk.Tk()
            root.withdraw()
    ...
    else: # train_model_choice is False
        print("\n--- 加载现有模型 ---")
            ...
            if predictor.X_test.size > 0:
                ...
                if needs_auto_retrain_opt_loaded:
                    ...
                    if optimized_model_tuned:
                        ...
                        print("对已加载模型的自动优化失败。")
                # else: # 不需要自动优化，optimized_predictions 和 eval_optimized 保持为初始的NaN状态
            else:
    ...
    if predict_future_choice:
        if model_to_use:
            print("\n--- 未来所有指标预测 ---")
            predictor.predict_future(model_to_use, num_steps=72, forecast_file_name="南京_AQI_Data_forecast.xlsx")
        else:
